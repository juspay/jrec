...
type family Sels fields where
  Sels '[] = U1
  Sels '[name := value] = Sel name value
  Sels ((name := value) ': xs) = Sel name value :*: Sels xs
...
instance
  ( FromNative (RecordRep fields) fields,
    ToNative (RecordRep fields) fields
  ) =>
  Generic (Rec fields)
  where
  type Rep (Rec fields) = RecordRep fields
  from r = toNative' r
  to rep = fromNative' rep
